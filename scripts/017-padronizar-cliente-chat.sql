...
DECLARE
  coluna_legada text;
BEGIN
  FOREACH coluna_legada IN ARRAY ARRAY['role', 'remetente', 'tipo', 'mensagem', 'conteudo', 'message', 'content']
  LOOP
    IF EXISTS (
      SELECT 1 FROM information_schema.columns
      WHERE table_schema = 'public'
        AND table_name = 'cliente_chat'
        AND column_name = coluna_legada
    ) THEN
      EXECUTE format(
        'ALTER TABLE public.cliente_chat ALTER COLUMN %I DROP NOT NULL',
        coluna_legada
      );
    END IF;
  END LOOP;
